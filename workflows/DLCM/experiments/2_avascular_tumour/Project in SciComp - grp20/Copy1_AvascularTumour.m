...
%                   [1./dM(sdof); ...
%                    BC1*bc1_vector; BC2*ones(size(idof2_))], ... % ; 
...
hold on;
for kk = 1:2 % size(inspect_rates,1)
    plot(tspan,inspect_rates(kk,:),'DisplayName',rate_names{kk});
end
grid on;
hold off;
xlabel('time')
...
saveData = struct('U', {U}, 'Usave', {Usave}, 'tspan', {tspan}, ...
    ...
    'doRadiusBC', {doRadiusBC}, 'max_radius', {max_radius}, 'Ne', {Ne}, ...
