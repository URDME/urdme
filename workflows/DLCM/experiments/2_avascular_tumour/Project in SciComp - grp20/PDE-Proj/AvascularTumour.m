...
cons = 0;%0.005;        % consumption of oxygen by cells
...
cutoff_die = 1000000000;    % the maximum amount of oxygen where cells can die
r_die = 0;%-0.025;        % rate of death
r_degrade = -0.1;     % rate of degradation for already dead cells
...
while tt <= tspan(end)
  ...
  N2=full(sum(Usave{i}==2))
  oxygen=Oxy(Adof)
  % classify the DOFs
  adof = find(U|U_dead); % all filled voxels (U_dead not necessary if U=-1)
  % singularly occupied voxels on the boundary: (will not be a source of moving cells)
  bdof_m = find(N*(U ~= 0) < neigh & ((U > 0 & U <= 1) | U_dead > 0));
  ...
  Oxy = full(fsparse([extdof; adof],1, ...
                     [ones(size(extdof)); ... 
                      -cons*full(max(U(adof),0)./dM(adof))], ... %change for dependence of U
                     [size(OLa.X,1) 1]));
...
ind_die = find(U(Adof)>0 & Oxy(Adof) < cutoff_die);
dead_conc = r_die*U(ind_die)*dt;
U(ind_die) = U(ind_die) + dead_conc;
U_dead(ind_die) = U_dead(ind_die) - dead_conc;
U(U(ind_die)<=0) = -1;

  Nalivegdgd=full(sum(Usave{i}>0))
  N2dghdh=full(sum(Usave{i}==2))
...
%     U(ix) = 0;
    ...
    U_dead(ddof) = U_dead(ddof) + r_degrade*U_dead(ddof)*dt;
