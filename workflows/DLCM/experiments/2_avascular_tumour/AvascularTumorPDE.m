...
warning('off', 'MATLAB:polyshape:repairedBySimplify')
 ...
 cont_cutoff = 0.95;          % contour size cutoff
 curv_cutoff = 2/(Nvoxels-1); % curvature larger than 1/curv_cutoff is clipped.
 
 % Savitzky-Golay Filter (sgf) parameters for curvature
 sgf_wind = 21; 
 sgf_deg = 7;
 ...
 [V,R] = mesh2dual(P,E,T, 'voronoi');   % can be changed afterwards for plotting

 D = 1; % D_rate, the rate with which cells move in the domain. 

 % simulation interval
 n_measurements = 10*Tend+1;    % ~ 5-10 measurements per time unit seems OK
 ...
 %#### Wrap into function for finding unstable modes given parameters?
...
reg_char = RegionalDynamics2D(par, init_volume, linspace(0,long_time,1000)); 
 ...
 U = fsparse(ii(:),1,start_value, [Nnodes 1]); % intialize
 ...
 [~,dM,~] = dt_operators(P,T);       %N gives the neighbours 

 [L,M] = assema(P,T,1,1,0); % assemble typical Laplacian instead! no lumping!
 ...
 while tt <= tspan(end)
     ...
     Adof = union(adof, idof); %[adof; idof];
     ...
     Pr(ndof_) = -mu_death/D.*dM(ndof); % 'manually' set sinks for dying dofs
     ...
     %% get move-'rates' (finite-volume approach)
      ...
      % Finite-volume approach - akin to the Gillespie-rates in AvascularTumor.m
      ...
      grad_loss = fsparse(ii,1, ...
          min((PR(jj_)-PR(ii)).*full(U(ii))/(hmax^2), 0), ... %outgoing flow
                     Nnodes);
      grad_gain = fsparse(ii,1, ...
          max((PR(jj_)-PR(ii)).*full(U(jj_))/(hmax^2), 0), ... %incoming flow
                     Nnodes);
 ...
 %%  Calculate timestep dt 
     ...
     % CFL condition
     % dt1 not sufficient for large enough surface tensions
     dt1 = 0.1 / max(abs(mover));  % ~CFL if D*gradquotient = 1 
     dt2 = Tend/n_measurements; %
     dt3 = 1*hmax;    % ensures rough stability approximations for FV scheme
     dt = min(min(dt1,dt2), dt3);
     %dt = dt1;
     DT(i) = dt; % save the time steps
     
     % DEBUGGING: if system equilibrium reached
     if dt == Inf
     ...
     if tspan(i+1) < tt+dt
         ...
         ZZ(:) = 0;
         ZZ(pdof) = 1;
         volp = sum(M*ZZ);      % volume, proliferating region (< total volume!)
         ZZ(:) = 0;
...
return
 ...
 %% Save data & animate
 saveData = struct('U', {U}, 'VU', {VU}, 'Usave', {Usave}, 'Prsave', {Prsave}, ...
     'Oxysave', {Oxysave}, 'tspan', {tspan}, ...
     'R', {R}, 'V', {V}, 'N', {N}, ...
     'Pr', {Pr}, 'Adof', {Adof},'adof', {adof}, 'adof_', {adof_}, ...
     'idof', {idof},'idof_', {idof_}, 'Nnodes',{Nnodes}, ...
     'P', {P}, 'noise', {noise}, 'rng_seed', {seed}, ...
     'rp', {rp}, 'rq', {rq}, 'rn', {rn}, 'k', {k}, 'hmax', {hmax}, ...
     'Tend', {Tend}, 'all_parameters', {allpar}, 'roundness', {roundness}, ...
     'radius', {radius}, 'BFTsave', {BFTsave});
 filename = "PDE_sim1";
