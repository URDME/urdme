...
% small cell clusters without a proper boundary, no infinite rates induced
...
sgf_wind = 21;  % sgf parameters for curvature
sgf_deg = 7;
...
%dM = dM./3;     % trapezoidal rule for mass lumping 
...
while tt <= tspan(end)
  ...
  [bdof_m_both_,bdof_m_live_,sdof_,sdof_m_,idof1_,idof2_,idof_,adof_] = ...
      map(Adof_,Adof,bdof_m_both,bdof_m_live,sdof,sdof_m,idof1,idof2,idof,adof);
  ...
  if updLU
    ...
    [cpos, ~] = contour(X, Y, Z, [0.5, 0.5]); % 0.5 works for doubly occ.??
    %cn = cpos(2,1);    
    ...
    %idof = idof(keep);           % idof reevaluated at each step...
    %idof_ = idof_(keep);         % so remember 'keep' instead of
                                    % altering idof again
  ...
  % Do mapping again
  movingb1 = 0; % set to zero if no such dofs exist
...
% plot 3 images at times tstamps
...
for iii = tstamps
  ...
  ndof = find(Usave{iii} == - 1);  % region of necrotic cells
...
%% Save data & animate
 saveData = struct('U', {U}, 'VU', {VU}, 'Usave', {Usave}, 'Prsave', {Prsave}, ...
     ...
     'P', {P}, 'roundness', {roundness}, 'radius', {R0}, ...
     'rp', {rp}, 'rq', {rq}, 'rn', {rn}, 'hmax', {hmax}, ...
