...
void mexFunction(int nlhs,mxArray *plhs[],int nrhs,const mxArray *prhs[])
{
  ...
  size_t i;
  for (i = 0; i < Ndofs; i++) u0[i] = (int)u0_double[i];
  for (i = 0; i < Ncells; i++) sd[i] = (int)sd_double[i];
  for (i = 0; i < nnzN; i++) prN[i] = (int)prN_double[i];
  ...
  /* seed */
  if (!mxIsNaN(*mxGetPr(mxSEED))) srand((unsigned)*mxGetPr(mxSEED));
  ...
  for (i = 0; i < Ndofs*tlen; i++) U_out[i] = (double)U[i];
  mxFree(U);
}
/*----------------------------------------------------------------------*/
